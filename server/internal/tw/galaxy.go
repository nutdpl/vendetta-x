...
func (s *Store) seedGalaxy() error {
	var n int
	if err := s.db.QueryRow(`SELECT COUNT(*) FROM tw_sectors`).Scan(&n); err != nil {
		return err
	}
	if n > 0 {
		return nil
	...
	for i := 1; i <= TotalSectors; i++ {
		...
		if _, err := tx.Exec(`INSERT INTO tw_sectors (id, name) VALUES (?, ?)`, i, name); err != nil {
			return err
		}
	}
	...
	link := func(a, b int) error {
		...
		_, err := tx.Exec(`INSERT OR IGNORE INTO tw_warps (frm, dst) VALUES (?, ?), (?, ?)`, a, b, b, a)
		return err
	}
...
func insertPort(tx *sql.Tx, sector, class int) error {
	...
	_, err := tx.Exec(`INSERT INTO tw_ports (sector, class, ore, org, equ) VALUES (?, ?, ?, ?, ?)`,
...
func (s *Store) Sector(id int) (*Sector, error) {
	var sec Sector
	err := s.db.QueryRow(`SELECT id, name FROM tw_sectors WHERE id = ?`, id).Scan(&sec.ID, &sec.Name)
	...
	rows, err := s.db.Query(`SELECT dst FROM tw_warps WHERE frm = ? ORDER BY dst`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) WarpExists(from, to int) (bool, error) {
	var n int
	err := s.db.QueryRow(`SELECT COUNT(*) FROM tw_warps WHERE frm = ? AND dst = ?`, from, to).Scan(&n)
	return n > 0, err
}
...
func (s *Store) PortAt(sector int) (*Port, error) {
	var p Port
	err := s.db.QueryRow(`SELECT sector, class, ore, org, equ FROM tw_ports WHERE sector = ?`, sector).
...
func (s *Store) SavePort(p *Port) error {
	_, err := s.db.Exec(`UPDATE tw_ports SET ore = ?, org = ?, equ = ? WHERE sector = ?`,
...
func (s *Store) FightersAt(sector int) ([]SectorFighters, error) {
	rows, err := s.db.Query(`SELECT owner, qty FROM tw_fighters WHERE sector = ? AND qty > 0 ORDER BY qty DESC`, sector)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SetFighters(sector int, owner string, qty int) error {
	if qty <= 0 {
		_, err := s.db.Exec(`DELETE FROM tw_fighters WHERE sector = ? AND owner = ? COLLATE NOCASE`, sector, owner)
		return err
	}
	_, err := s.db.Exec(`INSERT INTO tw_fighters (sector, owner, qty) VALUES (?, ?, ?)
