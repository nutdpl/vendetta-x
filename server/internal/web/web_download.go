...
//
//	/dl/<base64url(payload)>.<base64url(hmac)>     payload = "<fileID>:<expiryUnix>"
//
// The signature binds the file id and expiry, so a link can't be altered to
// point at another file or outlive its window.
func (s *server) signDownload(fileID int64, ttl time.Duration) string {
	payload := strconv.FormatInt(fileID, 10) + ":" + strconv.FormatInt(time.Now().Add(ttl).Unix(), 10)
...
// verifyDownload validates a token and returns the file id it authorizes.
func (s *server) verifyDownload(token string) (int64, bool) {
	...
	if !hmac.Equal(sig, mac.Sum(nil)) {
		return 0, false // forged or tampered
	}
	...
	if time.Now().Unix() > exp {
		return id, false // expired (id returned only for messaging; treated invalid)
	}
	return id, true
}
...
func (s *server) download(w http.ResponseWriter, r *http.Request) {
	id, ok := s.verifyDownload(r.PathValue("token"))
	if !ok {
	...
	if err := s.st.IncDownload(id); err != nil {
